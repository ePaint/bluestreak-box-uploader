"""Build script for Bluestreak Box Uploader.
...
def create_github_release(version: str, installer_path: Path) -> bool:
    ...
    tag = f"v{version}"
    title = f"v{version}"
    notes = f"""## Bluestreak Box Uploader {tag}

Upload Bluestreak MES/QMS certifications to Box.com for customer sharing.

### Features
- Query certifications by Order ID
- Select and upload certification documents
- Automatic Box folder structure creation
- Progress tracking and logging

### Requirements
- Windows 10/11
- Box JWT application credentials
- Network access to Bluestreak SQL Server
...
def main() -> int:
    ...
    if create_release:
        installer_path = project_root / "installer_output" / f"BluestreakBoxUploader_Setup_{new_version}.exe"
